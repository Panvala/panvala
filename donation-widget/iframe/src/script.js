...
class Donate {
  constructor() {
    ...
    this.amountInput.oninput = e => this.onAmountChange(e);

    this.assetSelect = document.querySelector('select');
    this.assetSelect.onchange = e => this.onAssetChange(e);

    this.form = document.querySelector('form');
    this.form.onsubmit = e => this.onConnectWalletOrSend(e);
    ...
    this.asset = this.props.defaultAsset || 'ETH';
    this.amount = Number(this.props.defaultAmount) || 0.1;
    this.form.asset.value = this.asset;
    this.form.amount.value = this.amount;
  }

  onConnectWalletOrSend(e) {
    e.preventDefault();
    e.stopPropagation();
  ...
  async handleMessageBound(evt) {
    ...
    switch (msg.type) {
      ...
      case 'sent': {
        ...
        await sleep(3000);
        this.postMessageToParentWindow('complete', msg.payload);
        break;
      }
